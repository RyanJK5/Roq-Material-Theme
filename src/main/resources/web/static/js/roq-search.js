(function () {
  function getConfig() {
    var node = document.getElementById("__config");
    if (!node) return {};
    try {
  ...
  function tr(key, fallback, count) {
    var map = (config && config.translations) || {};
    var raw = map[key] || fallback || "";
    if (typeof count === "number") return raw.replace("#", String(count));
    return raw;
  }
  ...
  function docs() {
    var nodes = Array.prototype.slice.call(
      (document.getElementById("roq-search-docs") || document).querySelectorAll(
        "#roq-search-docs [data-url]",
      ),
    );
    var out = nodes.map(function (node) {
      var abs = toAbs(node.getAttribute("data-url") || "");
      return {
        title: n(
          node.getAttribute("data-title") || node.getAttribute("data-url"),
        ),
        url: abs,
      };
    });
    out.push({
      ...
      url: toAbs(window.location.href),
    });
    return dedupe(out).filter(function (d) {
      return d.title && d.url && isSameOrigin(d.url);
    });
  }
  ...
  function snippet(node) {
    ...
    Array.prototype.slice
      .call(clone.querySelectorAll("code"))
      .forEach(function (c) {
        c.replaceWith(
          document.createTextNode("`" + (c.textContent || "") + "`"),
        );
      });
  ...
  function records(doc, pageUrl, pageTitle) {
    var article =
      doc.querySelector(".md-content__inner.md-typeset") ||
    ...
    var list = [
      {
        ...
        title: title,
        url: pageUrl,
        text: snippet(article.querySelector("p") || article),
        searchText: n(article.textContent || ""),
      },
    ];
    Array.prototype.slice
      ...
      .forEach(function (h) {
        ...
        while (next && /^(H2|H3|H4)$/i.test(next.tagName))
          next = next.nextElementSibling;
        var parts = [n(h.textContent || "")],
          w = h.nextElementSibling;
        while (w && !/^(H2|H3|H4)$/i.test(w.tagName)) {
          parts.push(n(w.textContent || ""));
          w = w.nextElementSibling;
        }
        list.push({
          pageTitle: title,
          sectionTitle: n(h.textContent || ""),
          title: title,
          url: pageUrl + "#" + h.id,
          text: snippet(next || h),
          searchText: n(parts.join(" ")),
        });
      });
    return list;
  }
  ...
  function runSearch(q) {
    ...
    var phrase = analyzed
      ...
      .map(function (x) {
        return {
          score:
            100 +
            (x.title.indexOf(query) !== -1 ? 10 : 0) +
            (x.text.indexOf(query) !== -1 ? 6 : 0),
          item: Object.assign({}, x.item, { missingTerms: [] }),
        };
      })
      .sort(function (a, b) {
    ...
    return analyzed
      ...
      .map(function (x) {
        return {
          score: x.matched * 10 - x.missing.length * 2,
          item: Object.assign({}, x.item, { missingTerms: x.missing }),
        };
      })
      .sort(function (a, b) {
        return b.score - a.score;
  ...
  function focusSnippet(item, q) {
    ...
    var source = n(item.searchText || preview),
    ...
    var start = Math.max(0, idx - 90),
      end = Math.min(source.length, start + 220);
  ...
  function resultArticle(item, q, extra) {
    ...
    hMain.innerHTML = mark(
      extra
        ? item.sectionTitle || item.pageTitle || item.title
    ...
    if (!extra && item.sectionTitle) {
      ...
      h2.innerHTML = mark(item.sectionTitle, q);
      article.appendChild(h2);
    }
    var p = document.createElement("p");
    p.innerHTML = mark(focusSnippet(item, q) || item.url, q);
    article.appendChild(p);
    var miss = missingHtml(item.missingTerms);
    if (miss) {
  ...
  function render(items, q) {
    ...
    group(items).forEach(function (g) {
      ...
      a.href = g.primary.url;
      a.appendChild(resultArticle(g.primary, q, false));
      li.appendChild(a);
      if (g.extras.length) {
        var more = document.createElement("details");
        more.className = "md-search-result__more";
        var summary = document.createElement("summary");
        var div = document.createElement("div");
        div.textContent =
          g.extras.length === 1
            ? tr("search.result.more.one", "1 more on this page")
            : tr(
        ...
        g.extras.forEach(function (e) {
          ...
          ea.className = "md-search-result__link";
          ea.href = e.url;
          ea.appendChild(resultArticle(e, q, true));
          more.appendChild(ea);
        });
        li.appendChild(more);
      }
  ...
  function onSearch() {
    ...
    ensureIndex().then(function () {
      ...
      if (!matches.length) {
        status.textContent = tr("search.result.none", "No matching documents");
        listEl.innerHTML = "";
        return;
      }
      status.textContent =
        matches.length === 1
          ? tr("search.result.one", "1 matching document")
          : tr("search.result.other", "# matching documents", matches.length);
      render(matches, q);
    });
  }
